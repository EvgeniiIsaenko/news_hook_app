...
void main() => runApp(const NewsHookApp()); // main method which starts this app

class NewsHookApp extends StatelessWidget {
  // class which point to the Home Page of the app
  const NewsHookApp({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          title: Text('News Hook',
              style: TextStyle(fontSize: 25, color: Colors.white))),
      body: _pages.elementAt(_currentPage),
      bottomNavigationBar: BottomNavigationBar(
        // navbar at the bottom
        onTap: (int index) {
          // sets the page that we want to see
          setState(() {
            _currentPage = index;
          });
        },
