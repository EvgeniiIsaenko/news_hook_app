...
class EventDescription extends StatelessWidget {
  // the class of the item  in the event list
  const EventDescription({super.key, required this.event});
  final Event event;

  Row timestamptzToText(Event event) {
    // parsing the timestamptz data type from the database
    ...
    return Row(
      // this displays the dateTime and time of the event
      children: [
        Column(
          children: [
            Container(
  ...
  @override
  Widget build(BuildContext context) {
    // all items in the event list are made here
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => EventPageOnPush(event: event)));
      },
      child: Row(
        children: <Widget>[
          timestamptzToText(event), // time of the event
          Container(
            // title of the event and short description
...
class EventsList extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // creates the list of the events
    return ListView.separated(
        ...
        itemBuilder: (context, int index) {
          print('item shown');
          return Container(
            // even though this is highlighted as unnecessary use: this is very much necessary as the future builder will ignore the description without it being in a container
            child: EventDescription(
                event:
                    events[index]), // this is the item that goes into the list
          );
        });
  }
}
    ...
    return ListView(
      children: [
        Container(
          ...
          child: Text('Ближайшие мероприятия:',
              style: TextStyle(
                  fontSize: 25,
                  color: Colors
        ...
        FutureBuilder<List<dynamic>>(
          // builds this widget in the future
          future: events,
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              ...
              return EventsList(
                  events: snapshot
                      .data!); // calls the class to create a list of all of the events
            } else {
              return Padding(
...
class EventPageOnPush extends StatelessWidget {
  // this is the widget class for the page that displays the event image, description, name, time etc.
  ...
  Column printTimeOfEvent(Event event) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Дата начала: $day.$month.$year',
            style: TextStyle(color: Colors.white, fontSize: 20)),
        Text('Время начала: ${parsedTime[0]}:${parsedTime[1]}',
            style: TextStyle(color: Colors.white, fontSize: 20))
      ],
    );
  }
  ...
  @override
  Widget build(BuildContext context) {
    // diplays event information (event's card)
    return Scaffold(
      ...
      body: ListView(
        children: [
          displayImage(
              event), // TODO: check with real urls to see if this works // TODO: add automatic resize
          Padding(
          ...
          Padding(
            padding: const EdgeInsets.only(left: 15.0),
            child: HtmlWidget(
              // transforms html format to a flutter widget format
              event.text,
